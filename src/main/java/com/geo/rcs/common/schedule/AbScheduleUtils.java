...
public class AbScheduleUtils {
    private final static String JOB_NAME = "AB_TASK_";
    ...
    public static void createAbTestJob(Scheduler scheduler, AbTest abTest) {
        try {
            //构建job信息
            JobDetail jobDetail = JobBuilder.newJob(AbTestJob.class).withIdentity(getJobKey(abTest.getJobId())).build();
